param(
  [string]$puppetVersion = '2015.3.2',
  [string]$puppetMaster = 'puppet.ldc.poc'
)
$MsiUrl = "https://pm.puppetlabs.com/puppet-enterprise/$puppetVersion/puppet-enterprise-$puppetVersion.msi"
$MsiUrlx64 = "https://pm.puppetlabs.com/puppet-enterprise/$puppetVersion/puppet-enterprise-$puppetVersion-x64.msi"
$PuppetInstallerPath = 'c:\vagrantshared\resources\installers'
$PuppetInstallerFile = 'puppet-enterprise.msi'
if ([System.IntPtr]::Size -eq 8) {
  Write-Host "Going Puppet 64-bit."
  $MsiUrl = $MsiUrlx64
  $PuppetInstallerFile = 'puppet-enterprise-x64.msi'
}
...
if (!($PuppetInstalled)) {
  ...
  $install_args = @("/qn", "/norestart", "/i", "$PuppetInstaller", "PUPPET_MASTER_SERVER=$puppetMaster")
