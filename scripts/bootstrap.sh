...
install_master() {
  ...
  /opt/puppetlabs/bin/puppet config set --section main dns_alt_names puppet,puppet.$(/opt/puppetlabs/bin/facter domain | tr '[:upper:]' '[:lower:]'),$(/opt/puppetlabs/bin/facter hostname),$(/opt/puppetlabs/bin/facter fqdn | tr '[:upper:]' '[:lower:]')
